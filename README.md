...
mounted path through Macker. Gateway handoffs use macOS's canonical `locallocks` plus `resvport` and the handoff-provided NFS server port because
the Ganesha endpoint does not expose `rpc.statd`; macOS legacy `-L/-P/-T/-3`
flags are used for this gateway path only. Direct CSI NFS options are not
rewritten. This remains experimental
...
paths/options, and Longhorn RWO/block PVs remain Pending. With a recent Macker, maclet records the actual exit code and
