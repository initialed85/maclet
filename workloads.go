...
func (m *workloadManager) runArgs(pod Pod, container ContainerSpec, managed *managedWorkload) ([]string, error) {
	...
	for _, mount := range container.VolumeMounts {
		...
		// default. Native Darwin workloads do not receive the projected Linux
		// token volume, but ignoring this automatic mount lets otherwise simple
		// Pods run without requiring automountServiceAccountToken: false.
		if mount.MountPath != "/var/run/secrets/kubernetes.io/serviceaccount" {
			return nil, fmt.Errorf("container %q declares volumeMount %q, which maclet/Macker does not support yet", container.Name, mount.MountPath)
		}
	}
