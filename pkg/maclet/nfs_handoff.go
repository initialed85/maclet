...
func resolveNFSVolumeForPod(ctx context.Context, client *APIClient, pod Pod, volume Volume) (genericNFSVolume, error) {
	...
	keys := []string{nfsHandoffServerAnnotation, nfsHandoffExportAnnotation, nfsHandoffVersionAnnotation, nfsHandoffMountPortAnnotation, nfsHandoffGenerationAnnotation}
	present := 0
	...
	return genericNFSVolume{
		...
		Server:       server,
		Share:        share,
		MountOptions: []string{"mountport=" + strconv.Itoa(mountPort), "vers=3", "tcp", "nolock", "resvport"},
